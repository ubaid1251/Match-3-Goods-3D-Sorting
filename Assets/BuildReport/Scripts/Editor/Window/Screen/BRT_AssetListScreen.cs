			...
			if (list != null) {
				...
				if (assetListToUse != null) {
					if (assetListToUse.Length == 0) {
                        ...
                        if (GUILayout.Button("Sync Compression Settings")) {
							foreach (var t_path in assetListToUse) {
								try {
									TextureImporter imp = AssetImporter.GetAtPath(t_path.Name) as TextureImporter;
									var s = imp.GetPlatformTextureSettings("Android");
									var i = imp.GetPlatformTextureSettings("iPhone");
                                    if (s.overridden) {
										i.overridden = true;
										i.maxTextureSize = s.maxTextureSize;

										i.textureCompression = s.textureCompression;
										i.compressionQuality = s.compressionQuality;
										if((s.format == TextureImporterFormat.DXT1 ||
											s.format == TextureImporterFormat.DXT1Crunched ||
											s.format == TextureImporterFormat.DXT5 ||
											s.format == TextureImporterFormat.DXT5Crunched)) {
											s.format = TextureImporterFormat.ASTC_8x8;
											s.crunchedCompression = false;
										}
										i.format = s.format;
										imp.SetPlatformTextureSettings(i);
									}
									else if (i.overridden) {
										s.overridden = true;
										s.maxTextureSize = s.maxTextureSize;
										s.textureCompression = i.textureCompression;
										s.compressionQuality = i.compressionQuality;
										s.format = i.format;
									...
									Debug.Log(t_path + " : " + e.Message);
								}
                                finally {
                                    try {
										AudioImporter imp = AssetImporter.GetAtPath(t_path.Name) as AudioImporter;
										if(imp.ContainsSampleSettingsOverride("Android")) {
											var s = imp.GetOverrideSampleSettings("Android");
											imp.SetOverrideSampleSettings("iPhone", s);
										}
										else if(imp.ContainsSampleSettingsOverride("iPhone")) {
											var i = imp.GetOverrideSampleSettings("iPhone");
											imp.SetOverrideSampleSettings("Android", i);
										}
									}
                                    catch(Exception e) {
										Debug.Log(t_path + " : " + e.Message);
